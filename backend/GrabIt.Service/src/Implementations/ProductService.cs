...
namespace GrabIt.Service.Implementations
{
    public class ProductService : BaseService<Product, ProductReadDto, ProductCreateDto, ProductUpdateDto>, IProductService
    {
        ...
        {
            if (categoryId.Equals(Guid.Empty)) throw ErrorHandlerService.ExceptionArgumentNull($"{nameof(categoryId)} can't be empty or null.");
        ...
        {
            if (
                createData.Title == null ||
                createData.Description == null ||
                createData.Price <= 0 || createData.Stock <= 0
                ) throw ErrorHandlerService.ExceptionArgumentNull($"While Creating new Product title, discription, price, stock can't be null.");
